...
def build_provider(
    ...
    if name in {"edge", "litert", "local"}:
        endpoint = base_url or settings.cloud_base_url or ("http://127.0.0.1:9379" if name == "litert" else None)
        if not endpoint:
            raise ValueError(f"{name} needs an HTTP server URL; use cogni setup or cogni login {name} --base-url URL.")
    ...
    raise ValueError(f"Unknown provider '{name}'. Supported providers: ollama, openai, anthropic, google, grok, meta, litert, edge, local, stub")
